...
#define MAX_NEGATE 1001                           // 이를 위해 먼저 negate() 함수를 구현해야 할 것이다.
...
{                                                                      // Polynomial negate sub( Polynomial a, Polynomial b);               // a-b를 구해 반환하는 함수
    ...
    float coef2[MAX_NEGATE];                          // 2번 - 두 다항식의 곱(c = a * b)을 구하는 함수 mult()를 구현하라.
...
{                                                                    // 이 함수의 매개변수로는 반드시 포인터형을 사용해야한다.
    return p.coef[i], p.coef2[i];                                       // 포인터는 5장에서 자세히 다루는데, 이를 참고해서 포인터를 사용해야하는 이유를 생각해 보라.
...
Polynomial sub(Polynomial a, Polynomial b)  // 1번 두 다항식에 뺄셈을 구하는 코드
{
    Polynomial p;
