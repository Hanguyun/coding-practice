...
new_ai = [] # 일단 중복되는 원소를 뽑기 위해 ai 리스트를 따로 생성
x = [] # 중복되는 원소를 뽑기 위한 리스트
...
for i in range(2):  # 컴퓨터도 사용자와 마찬가지로 주사위를 3번 던지기 가능 처음 주사위를 던지고 시작 그래서 2번 반복
    if len(set(ai)) == 1:  # 중복이 되지 않는 세트의 특징을 이용하여 중복 제거가 되어 값이 1개만 남는다면 모든 값이 같다
        ...
        for j in ai: # for문을 통해 각 원소를 순회하며 처음 등잔한 값인지 여부를 판별, 이미 이전에 등장했던 원소들만 솎아내어 중복 원소만 추출
            if j not in x: # https://jimmy-ai.tistory.com/217 참조하여 제작
                    x.append(max(i)) ---------------------------------------------------------> 오류 부분 중복 되는 값을 추출하되 높은 값 추출해야함.
            else: # 이부분에 if i not in new_a : 가 있는데 그걸 지우면 중복되면 무조건 추가로 가능
                new_ai.append(i)
                total = len(new_ai) # len을 이용하여 몇 개가 중복되어 뽑혔는지 확인하고 total에 저장
                ai_reroll = random.choices(dice, k=5-total)  # 중복되어 있는 개수를 total에 저장했으므로 5-total은 비어있는 주사위 개수
                new_ai.extend(ai_reroll)  # ai_reroll 변수에 있던 정수를 new_ai 덱에 삽입
                ai.clear()# new_ai에 있던 값을 다시 ai로 옮기기 위하여 리스트에 모든 값을 제거 new_ai를 계속 쓰지 않는 이유는 위에 조건에서 계속 ai를 사용했기 때문에 복사
                ai = new_ai[:] # 얕은 복사를 이용하여 값을 이동 https://inkkim.github.io/python/%ED%8C%8C%EC%9D%B4%EC%8D%AC-%EB%A6%AC%EC%8A%A4%ED%8A%B8-%EB%B3%B5%EC%82%AC/ 참조
