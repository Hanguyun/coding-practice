...
for i in range(2):  # 컴퓨터도 사용자와 마찬가지로 주사위를 3번 던지기 가능 처음 주사위를 던지고 시작 그래서 2번 반복
    if len(set(ai)) == 1:  # 중복이 되지 않는 세트의 특징을 이용하여 중복 제거가 되어 값이 1개만 남는다면 모든 값이 같다
        ...
        for i in ai: # for문을 통해 각 원소를 순회하며 처음 등잔한 값인지 여부를 판별, 이미 이전에 등장했던 원소들만 솎아내어 중복 원소만 추출
            if i not in x: # https://jimmy-ai.tistory.com/217 참조
                x.append(i)
            else:
                if i not in new_ai :
                    new_ai.append(i)
                    ...
                    new_ai.append(ai_reroll)  # ai_reroll 변수에 있던 정수를 new_ai 덱에 삽입
