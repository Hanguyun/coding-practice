...
#define MAX_DEGREE 1001                            // 앞의 문제에서와 같이 c = a-b = a+(-b) 관계를 이용해 구현하라.
#define MAX_NEGATE 1001 //1번 문제                                 // 이를 위해 먼저 negate() 함수를 구현해야 할 것이다.
...
{                                                                      // Polynomial negate sub( Polynomial a, Polynomial b);               // a-b를 구해 반환하는 함수
    ...
    float coef[MAX_DEGREE];
                                                                        // 2번 - 두 다항식의 곱(c = a * b)을 구하는 함수 mult()를 구현하라.
} Polynomial;                                                   // Polynomial negate smult( Polynomial a, Polynomial b);

typedef struct                  //1 번 문제의 다항식 구조체를 선언
{
    int negate;
...
int negate(Polynomial2 q)       // 1번 문제
{
    return 0;
}
...
float coefficient2(Polynomial2 q, int j)            // 1번 문제
{
    return q.coef2[j];
}
...
float evaluate2(Polynomial2 q, float x)                       // 1번 문제
{
...
Polynomial2 sub(Polynomial2 a, Polynomial2 b)              // 1번 문제
{
    Polynomial2 q;
