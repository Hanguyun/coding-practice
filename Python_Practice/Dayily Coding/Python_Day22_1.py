'''
- 다음과 같이 블랙잭 게임을 하는 프로그램을 만들어 보자
...
7. 사용자가 카드를 더 받으면 카드를 추가로 받은 점수를 계산한다.
    (21점이 넘어가면 사용자 패배. 21점이면 자동으로 카드를 그만 받는다. 그 이외의 경우 5번으로 돌아가 반복)
8. 사용자가 카드 받기를 끝내면 컴퓨터의 카드 받기 시작
9. 사용자가 22점 이상의 점수로 패배한 상황이면 컴퓨터는 자신의 점수만 공개하고 승리
10. 사용자의 점수가 21점 이하면 컴퓨터는 카드를 공개하고 카드를 받기 시작한다.
