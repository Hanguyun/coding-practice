...
for y in range(2):  # 룰 = 컴퓨터도 사용자와 마찬가지로 주사위를 3번 던지기 가능 처음 주사위를 던지고 시작 그래서 2번 반복
    ...
    if len(ai) != len(set(ai)):  # ai(리스트) ai(세트)와 같지 않다면 즉 리스트는 중복 되면 사라지지 않지만 세트는 중복 불가라서 중복이 되면 숫자가 줄어든다 그렇다면 같지 않다가 성립 되면 중복되는 숫자가 있다는 뜻
    ...
    elif len(set(ai)) == 1:  # 중복이 되지 않는 세트의 특징을 이용하여 중복 제거가 되어 값이 1개만 남는다면 모든 값이 같다
        print(f"현재 ai패: {ai}")  # 룰 = 컴퓨터와 사람이 주사위를 던진 후, 던질때 마다 패를 표기
        break  # 룰 = 5개의 주사위의 숫자가 모두 같다면 더 이상 던지지 않는다.

    elif len(set(ai)) == 5:  # 룰 = 중복이 되지 않는 세트의 특징을 이용 중복 제거가 되지 않아 값이 5개일 경우 조건 성립 = 모두 다를때
        ...
        if ai == [1, 2, 3, 4, 5] or ai == [2, 3, 4, 5, 6]:  # 룰 = 스트레이트는 경우의 수가 2가지
            ...
            elif len(ai) != len(set(ai)):  # 룰 = 동일한 숫자가 생긴다면
                if 2 in count_num.values():  # 동일한 숫자의 값이 2 즉 같은게 2개라면
                    del ai[0:5]  # ai에 있는 패를 다 삭제 후
                    ai.append(key * 2)  # 키를 2개 삽입 고정하고
                    ai.extend(random.choices(dice, k=3))  # 주사위 3개 다시 뽑기
                    ai.sort()  # ai패를 보기 좋게 정렬하고 진행
                    print(f"현재 ai패: {ai}")  # 룰 = 컴퓨터와 사람이 주사위를 던진 후, 던질때 마다 패를 표기
                    continue # 위에 조건 만족 했으니 continue로 밑에 조건 건너뛰기
                elif 3 in count_num.values():  # 동일한 숫자의 값이 3개라면
                    del ai[0:5]  # ai에 있는 패를 다 삭제 후
                    ai.append(key * 3)  # 키를 3개 삽입
                    ai.extend(random.choices(dice, k=2))  # 주사위 2개 다시 뽑기
                    ai.sort()  # ai패를 보기 좋게 정렬하고 진행
                    print(f"현재 ai패: {ai}")  # 룰 = 컴퓨터와 사람이 주사위를 던진 후, 던질때 마다 패를 표기
                    continue  # 위에 조건 만족 했으니 continue로 밑에 조건 건너뛰기
                elif 4 in count_num.values():  # 동일한 숫자의 값이 4개라면
                    del ai[0:5]  # ai에 있는 패를 다 삭제 후
                    ai.append(key * 4)  # 키를 3개 삽입
                    ai.extend(random.choices(dice, k=1))  # 주사위 1개 다시 뽑기
                    ai.sort()  # ai패를 보기 좋게 정렬하고 진행
                    print(f"현재 ai패: {ai}")  # 룰 = 컴퓨터와 사람이 주사위를 던진 후, 던질때 마다 패를 표기
                    continue  # 위에 조건 만족 했으니 continue로 밑에 조건 건너뛰기
                elif 3 in count_num.values() and 2 in count_num.values():  # 동일한 숫자의 값이 3, 2라면
                    del ai[0:5]  # ai에 있는 패를 다 삭제 후
                    ai.append(key * 3)  # 키를 3개 삽입
                    ai.extend(random.choices(dice, k=2))  # 주사위 2개 다시 뽑기
                    ai.sort()  # ai패를 보기 좋게 정렬하고 진행
                    print(f"현재 ai패: {ai}")  # 룰 = 컴퓨터와 사람이 주사위를 던진 후, 던질때 마다 패를 표기
                    continue  # 위에 조건 만족 했으니 continue로 밑에 조건 건너뛰기
                else:  # 마지막 조건 2,1,2 or 2,2,1 or 1,2,2이라면
                    ai.sort()  # 오름차순 정렬은 자동으로 제일 높은수가 뒤로감
                    del ai[0:3]  # 0~2까지 삭제
                    ai.extend(random.choices(dice, k=3))  # 주사위 3개 다시 뽑기
                    ai.sort()  # ai패를 보기 좋게 정렬하고 진행
                    print(f"현재 ai패: {ai}")  # 룰 = 컴퓨터와 사람이 주사위를 던진 후, 던질때 마다 패를 표기
