...
Polynomial negate(Polynomial p)                  // 1번
{
    Polynomial j = p;
     j.degree = p.degree;
     for (int i = 0; i <= p.degree; i++) {
...
Polynomial sub(Polynomial a, Polynomial b) {                //1번
    Polynomial nb = negate(b);
    return add(a, nb);
}
Polynomial mult(Polynomial a, Polynomial b)                                      // 2번 //3번
{
    Polynomial p = {0};                    // p초기화 하지 않아서 쓰레기 값이 들어옴.
    int i, j;
...
void print_poly2(Polynomial p, char str[])           //1번
{
    printf(" %s", str);
    for (int i = p.degree; i > 0; i--)
        printf("%5.1f x^%d + ", p.coef[i], i);
...
void main()
{
    ...
    print_poly(b, " B = ");
    print_poly(c, "A+B = ");
    print_poly2(d, "1번 문제 A-B = ");
    print_poly3(e, "2, 3번 문제 A*B = ");
    //print_poly(f, "4번 최고차항 정리 전 E =")
    //print_poly(f, "4번 최고차항 정리 후 E =")
    printf("A(1)= %f\n", evaluate(a, 1.0f));
    printf("B(2)= %f\n", evaluate(b, 2.0f));
}
